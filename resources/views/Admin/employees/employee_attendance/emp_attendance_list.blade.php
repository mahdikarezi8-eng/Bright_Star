            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center flex-wrap mb-3">
                    ...
                    @if (Auth::user()->can('employee_attendance_add'))
                        <a href="{{ route('emp_attendance_add') }}" class="btn btn-primary btn-sm">
                            Take Attendance
                        </a>
                    @endif
                ...
                @endphp
                <form method="GET" action="{{ route('emp_attendance_list') }}" class="mt-3">
                    <div class="row mb-3">
                        <div class="col-md-3">
                    ...
                    <table id="data" class="table table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($employees as $employee)
                                <tr>
                                    <td class="py-3 px-3 text-center"">{{ $employee->id }}</td>
                                    <td class="py-3 px-3 text-center"">
                                        {{ $employee->first_name }} {{ $employee->last_name }}</td>
                                    <td class="py-3 px-3 text-center">
                                        {{ $employee->employee_attendances->count() }} Day
                                    </td>
                                    <td class="py-2 px-2 text-center">
                                        <a href="{{ route('emp_attendance_detail', ['id' => $employee->id, 'year' => $year, 'month' => $month]) }}"
